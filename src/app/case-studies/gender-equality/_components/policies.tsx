...
const slides = [
  ...
  },
  {
    ...
    description: "Implementing zero-tolerance policies against gender-based violence and harassment with clear reporting mechanisms and support systems for all team members.",
  ...
  },
  {
    ...
    description: "Ensuring promotion criteria are transparent and merit-based, with targeted professional development opportunities to support career advancement for all genders.",
  ...
  },
  {
    ...
    description: "Supporting both men and women with family responsibilities through flexible scheduling, remote work options, and parental leave policies that encourage shared caregiving.",
    altText: "Healthcare worker balancing professional and family responsibilities",
  },
];
  ...
  return (
    <div className="bg-gradient-to-br from-[#00205C] to-[#1A5632] text-white py-8 mb-12 md:mb-24">
      ...
      <div className="max-w-6xl mx-auto  relative w-full overflow-hidden ">
        <div className="relative flex transition-transform duration-500 ease-in-out h-[500px] pt-12 md:pt-0" style={{ transform: `translateX(-${currentSlide * 100}%)` }}>
          {slides.map((slide) => (
            <div key={slide.id} className="min-w-full relative px-2 mt-8 md:px-8">
              ...
              <div className="absolute top-[70%] md:top-1/2 transform left-[20%] md:left-[40%] bg-white p-3 md:p-12 w-[75%] md:max-w-[50%] mr-12">
                <div className="text-sm md:text-base text-black">{slide.description}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <button onClick={goToPrevSlide} className="absolute z-10 left-[75%] md:left-2 top-6 md:top-1/2 -translate-y-1/2 bg-blue-900 border border-white text-white rounded-full p-2 " aria-label="Previous slide">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="size-6">
            <path stroke-linecap="round" stroke-linejoin="round" d="M6.75 15.75 3 12m0 0 3.75-3.75M3 12h18" />
          </svg>
        </button>
        <button onClick={goToNextSlide} className="absolute right-2 top-6 md:top-1/2 -translate-y-1/2 bg-blue-900 border border-white  text-white  rounded-full p-2 " aria-label="Next slide">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="size-6">
        ...
        <div className="flex gap-2 mt-12 md:mt-6 w-full justify-center">
          {slides.map((_, index) => (
            <button key={index} onClick={() => goToSlide(index)} className={`w-3 h-3 rounded-full ${currentSlide === index ? "bg-white" : "bg-white/40"}`} aria-label={`Go to slide ${index + 1}`} />
          ))}
        </div>
