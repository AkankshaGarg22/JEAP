...
const Operation = () => {
  ...
  function isMobile() {
    // Simplified regular expression to detect common mobile device patterns
  ...
  useEffect(() => {
    if (isVisible) {
      if (isMobile()) {
        setMobileImageSrc("/assets/animations/LineAnimationsMobile.svg"); // Your mobile image path
      } else {
        setImageSrc("/assets/animations/LineAnimation1.svg");
      }
    }
  }, [isVisible, imageSrc, mobileImageSrc]);

  return (
      <div id="operation" className="min-h-screen flex flex-col justify-evenly items-center pt-16 font-CompasseRegular md:pt-6">
      ...
      <div ref={ref} className={"py-8 md:pt-2"}> 
        {imageSrc && !isMobile() && <img src={imageSrc} alt="Image Loaded on Scroll" />}
        {mobileImageSrc && isMobile() && <img src={mobileImageSrc} alt="Mobile Image" />}
      </div>
