...
const Mission = ({ isVisible }: { isVisible: boolean }) => {
  ...
  // ${isVisible ? "rounded-none" : "[clip-path:circle(50%_at_50%_0%)]"}
  return (
    <div
      id="mission"
      className={
        `min-h-screen flex flex-col items-center justify-start xl:justify-evenly bg-cover bg-center lg:bg-contain bg-no-repeat  z-[200] 
        bg-[#1a5632] text-white ${isVisible ? "rounded-none" : "rounded-t-[669px]"}
        `}
    >
