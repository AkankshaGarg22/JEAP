...
const SelectedTextComponent = ({ selected }: { selected: string }) => {
  ...
  return (
    <div key={key} className="basis-5/6 md:basis-1/2 text-start md:text-start text-wrap text-[16px]  md:text-lg leading-tight md:leading-[1.05] xl:text-lg xl:leading-[1.25] px-5  animate-slide-in" dangerouslySetInnerHTML={{ __html: selected }} />
  );
};
...
export default function Carousel({ item, autoSlide = false, autoSlideInterval = 3000 }: CarouselProps) {
  ...
  return (
    <div className="overflow-hidden relative h-full">
      ...
      <div className="absolute inset-0 md:top-[100px] xl:top-[125px]  bg-transparent text-white flex flex-col md:flex-row items-center md:items-start justify-start md:justify-end xl:justify-center gap-4">
